...
| 11 | 直线上级体现 | 邮件内容按 Supervisor（直线上级）分组展示，主管一眼找到自己下属的清单；直线上级均已在 userID 中注明 Supervisor 权限（BK 列），本就在收件人内，不额外扩展收件人。直线上级取自 userID BI 列（索引 60，邮箱），按邮箱反查姓名展示 |
...
2. A 类汇总（与工作台/甘特图同源）：
   ...
   - 解析每人直线上级：userID BI 列(60) 取直线上级邮箱 → 按邮箱反查 userID NAME 得姓名；
     BI 为空 → 归入「未配置直线上级」分组
3. B 类汇总：
...
| Supervisor 分组 | 同一直线上级的多名下属未安排 → 出现在同一分组；BI 列为空的人员 → 「未配置直线上级」组 |
